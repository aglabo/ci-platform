...
URL 組み立てはテンプレート内のプレースホルダを解決済み変数で置換する変換方式です。
解決ルールは決定論的であり、同一入力に対して常に同一の出力を返す。`tool-version` は validate-inputs ステップで X.Y.Z 形式 (v プレフィックスなし) に確定済みであり、URL 組み立て時に `v` プレフィックスを付与する。
...
- `checksum-template`: `<filename>` メタ変数とリテラル文字 (`A-Za-z0-9._-`) で構成される文字列。省略時は `"checksums.txt"` を使用。`<filename>` は解決済みの `asset_filename` に置換される
...
| R-006   |    6 | `checksum_filename` を生成 (R-001 適用後のテンプレートから) 。`<filename>` が含まれる場合は R-004 で生成済みの `asset_filename` で置換する | checksum ファイル名文字列を出力                  |
...
| `checksum-template="<filename>.sha256"`, `asset_filename="actionlint_1.7.10_linux_amd64.tar.gz"` → `actionlint_1.7.10_linux_amd64.tar.gz.sha256`                                      | R-006 適用        | REQ-F-003 | `<filename>` を asset_filename で置換                     |
| `checksum-template="checksums.txt"` (リテラルのみ)                                                                                                                                    | R-006 適用        | REQ-F-003 | `<filename>` なし・リテラルのみも有効                     |
...
| 2026-03-23 | 1.2.0   | checksum-template 設計変更: `<filename>` メタ変数を導入 (DD-05) 。R-006 に `<filename>` → asset_filename 置換を追加。Input Domain・Edge Cases 更新                                  |
