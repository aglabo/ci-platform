...
GitHub Actions の composite action として、以下のアクションを実行する。

- 指定した GitHub リポジトリからの、ツールのダウンロード
- チェックサム検証
- アーカイブ展開

ツールバイナリを実行可能な PATH に配置する。

実装は関数ライブラリ式 (4 ファイル構成) で新規作成する。
既存スクリプト群 (`setup-directories.sh` 等) は参考実装として廃止する。

主要な設計決定:

- GitHub API (`api.github.com`) でアセット一覧を 1回取得し、tar.gz の arch 解決、および
...
- 複数値返却規約: 1行 = 1 値・空行なしで出力し、呼び出し元は `mapfile` で行単位にキャプチャする
...
- `detect_arch()`: `runner.arch` → `ARCH_CANDIDATES` を 1行1値で stdout に出力
...
- `DOWNLOAD_URL` (1行目) ・`CHECKSUM_URL` (2行目) ・`ARCH_SUFFIX` (3行目) を 1行1値で stdout に出力
...
- 正常系: 全関数が順に呼び出されること
- 異常系: 各フェーズ失敗時に、失敗にあった exit code で終了すること
...
- WHEN Commit 18 (`setup-tool.sh`) のテストが GREEN の場合に実施する
- 廃止予定スクリプトを削除:
...
- `runs.using: "composite"` で `setup-tool.sh` を 1回呼び出すだけのシンプルな構成
