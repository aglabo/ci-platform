...
ローカル開発から CI パイプラインまで、複数のレイヤーで独立したセキュリティ検証をします。
...
単一の検出ポイントへの依存を避け、ローカルと CI の 2 レイヤーで独立してセキュリティを検証します。
...
以下のファイルはセキュリティポリシーを定義しています。変更時は必ず理由を明記してください。
...
除外設定を安易に追加すると、実際のシークレット漏洩が発生するリスクがあります。
