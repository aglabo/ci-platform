...
> **最短で試したい場合** → [クイックスタート](./11-quickstart.ja.mdx)
...
詳細は [リファレンス](./13-reference.ja.mdx) を参照してください。

> Linux 系ランナー専用です。非 Linux ランナー (macOS・Windows) では処理が失敗します。

詳細は以下を参照してください。

- [Validate Environment 概要](./10-about-validate-environment.ja.mdx)
- [クイックスタート](./11-quickstart.ja.mdx)
- [利用シナリオ](./12-basic-scenarios.ja.mdx)
- [リファレンス](./13-reference.ja.mdx)
...
現在の実装状況の詳細は [ci-platform とは](./00-platform-overview.ja.mdx#️-現在の実装状況) を参照してください。
...
さらに詳細な使い方 (複数ジョブの組み合わせ・段階導入パターンなど) は [利用シナリオ](./12-basic-scenarios.ja.mdx) を参照してください。
...
- [ci-platform とは](./00-platform-overview.ja.mdx): プロジェクト概要と設計方針
