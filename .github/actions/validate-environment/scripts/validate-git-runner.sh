...
validate_git_runner() {
  ...
  if ! validate_os; then
    ...
    echo "message=Unsupported OS: ${DETECTED_OS} (Linux required)" >> "$(get_github_output_file)"
    exit 1
  fi
...
||||||| parent of 1fe6e6d (feat(actions/validate-environment): GitHub Actions ランナー検証スクリプトを追加)
=======
...
validate_git_runner
>>>>>>> 1fe6e6d (feat(actions/validate-environment): GitHub Actions ランナー検証スクリプトを追加)
